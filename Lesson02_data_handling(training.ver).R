...
popData <- read.csv(file, header=T) #제목이 있을 경우 header=T
class(popData)
popData$area = factor(popData$area, levels = c(1:7),
                      labels = c("수도권", "동남권","대경권", "충청권", "전라권", "강원권", "제주도")) #순서대로 이름 라벨링
popData
...
str(iris) #데이터구조확인
sqldf("select Species #select column이름
      from iris       #from 테이블이름 where 조건건
      where Species='setosa'") #iris 데이터셋에서 모든 column가져오고, Species가 setosa인 것만
iris.sql = sqldf("select distinct Species from iris") #distinct 하면 중복된 값은 제외하고 나옴
iris.sql
sqldf("select * from iris where Species='virginica'") # * : all
sqldf('select Species, sum("Sepal.Length") as SepalLength from iris group by Species') # as A : 이름을 A로, group by : 그룹별
sqldf('select Species, avg("Petal.Length") as avg, stdev("Petal.Length") as sd from iris group by Species')

#권역별 인구수
pop.area <- sqldf("select area,
                  sum(pop10) as area10, sum(pop18) as area18
                  from popData group by area") #area 그룹별로 10년도, 18년도 인구 합
tot10 <- sum(pop.area$area10) #2010년도 인구 합
tot18 <- sum(pop.area$area18) #2018년도 인구 합
pop.pnt10 <- pop.area$area10/tot10*100 #vector  #전체인구대비권역별인구
